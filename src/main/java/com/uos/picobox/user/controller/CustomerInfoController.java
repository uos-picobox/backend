...
import com.uos.picobox.user.service.InfoService;
...
@Tag(name = "00. 회원 - 회원 정보 수정", description = "회원 정보 수정 기능을 제공합니다.")
@RestController
...
public class CustomerInfoController {
    ...
    public ResponseEntity<?> getCustomerInfo(
        ...
        CustomerInfoResponseDto response = infoService.findCustomerInfoById(customerId);
    ...
    public ResponseEntity<?> updateCustomerInfo(
            ...
            @Parameter(hidden = true) @RequestHeader("Authorization") String sessionId) {
        CustomerInfoResponseDto response = infoService.updateCustomerInfo(dto);
        return ResponseEntity.ok(response);
    }
}
